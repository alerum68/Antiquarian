...
### Task 1: Audit and close Issue #5 (Scriptorium FIELD_REMAP relocation)
...
### Task 2: Extract testable normalize-and-validate call sites in Voyageur (Issue #3)
...
### Task 3: Wire `facts[]` into Archivist's church-flavor renderer (Issue #4)
...
### Task 4 (Manual checkpoint — NOT an implementer task): Live-verify `familysearch_census.yaml`
...
### Task 5 (Manual checkpoint — NOT an implementer task): End-to-end live verification and final issue closure
