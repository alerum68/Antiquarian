...
| 14 | ✅ | Ancestry Index-Panel-Data Extraction (`docs/superpowers/plans/2026-08-15-ancestry-index-panel-extraction.md`) - all 5 tasks complete, Task 5 live verification confirmed real `pid`/`household_id` from the API path and correct household grouping (Kosses, May) in generated GEDCOM. Tasks 1-3 (JS) had been implemented but never committed - committed now (`fbcc2b5`), along with a separate bundle of already-tested FS ark-prefix/citation fixes from the earlier Task 12/13 live verifications that were also left uncommitted (`2186033`). FamilySearch side also re-verified live on the same collection (17 records, 3 images, clean GEDCOM) at user's request - confirmed the `person_ark` double-prefix fix works correctly in real output. Post-verification fix: `SelfGender` returned the full word ("Male"/"Female") instead of M/F/U - `ancestryColumnsFromIndexPanelRecord()` now normalizes via `ancestryNormalizeGender()` (v0.3.28). Follow-on tracked: [issue #25](https://github.com/alerum68/Antiquarian/issues/25) (eliminate remaining FS Information-tab dependency). Plan marked COMPLETE. |
| 15 | ✅ | Ancestry Index-Panel-Data Field Coverage Extension (`docs/superpowers/plans/2026-08-15-ancestry-index-panel-field-coverage-extension.md`) - all 3 tasks complete. Task 3 live-verified on a real Canadian record (dbId `1578`/imageId `4396761_00576`, Ontario): 316 participants, 249 real `Religion` facts correctly emitted as `1 RELI` in GEDCOM. Review pass found and fixed 3 bugs in the adjacent, already-committed Complex Field Integration plan (row 16): Canadian birthplaces misclassified as "foreign" (no Canadian provinces in `is_foreign_birthplace()`'s allowlist - serious given this project's Canadian/Métis/HBCA subject base), a dropped `cap_case()` normalization in `get_occupation_value()`, and a `"Widowed1"→marital_status` guess that violated this plan's own "numbered-duplicate fields - do not guess" rule with a real silent-overwrite risk. Found but deferred to a new plan: Ancestry gathers hardcode `country="USA"` (Voyageur.js) and `"US/United States Federal Census"` (A.py, Census.py - the latter also drives the on-disk image folder name) regardless of actual source country; Commissioner's `CensusParticipantExtra` rejects `marital_status` outright even though it's been mapped since the original plan's Task 4. |
| 16 | ✅ | Complex Field Integration (`docs/superpowers/plans/2026-08-15-complex-field-integration.md`) - Tasks 1-3 complete and committed (`b6c2dbf`, `2b14665`, `f06c233`). Unified standard mappings added to Ancestry/FS schemas, dynamic occupation template with notes implemented in Archivist, foreign birthplace mapped to Nationality. All targeted and full suite tests pass. |
| 17 | ⏭️ | Census Pipeline Symmetry + Marriage Details (`docs/superpowers/plans/2026-08-15-census-pipeline-symmetry-and-marriage-details.md`) - plan written, not yet started. 5 tasks: (1) wire up dead dynamic-occupation fields - no field-map entries produce the raw columns `get_occupation_value()` reads, confirmed unreachable; (2) FS field-coverage symmetry - `fsColumnsFromCanonicalFields()` drops fields FS's own API already extracts; (3) fix Canadian country/collection-title mislabeling across `Voyageur.js`/`A.py`/`Census.py`; (4) extend `Commissioner/Paleographer/prompts/Census.pmt` to declare already-mapped-but-undeclared `type_specific_fields` keys; (5) new "Marriage Details" custom fact type (first-married date, # of marriages, widowed), following the Race precedent. |
| 24 | ✅ | Function Renaming (`docs/superpowers/plans/2026-08-15-function-renaming.md`) - expanded from a Gemini/Antigravity draft that only covered 3 Commissioner functions with unverified (partly wrong) caller lists. Audited Commissioner (all 25 functions), Voyageur, Archivist, Paleographer; confirmed 8 real candidates, including `cap_case` as 3 independent implementations (Commissioner, Archivist/Utils.py, Voyageur/census_schema.py) the original draft would have cross-contaminated with a blind text replace. All 8 renamed via PyCharm's semantic `rename_refactoring` tool (not AI-delegated text replace): `cap_case`→`capitalize_text_string` (×3), `parse_to_iso`→`parse_date_to_iso_format`, `validate_soft`→`validate_collection_softly`, `spouse_evaluation`→`evaluate_spouse_match`, `child_evaluation`→`evaluate_child_match`, `sex_code`→`normalize_sex_code`. Also cleaned up stale test names/comments left over from each rename (full-cleanup discipline). Full suite: 488 passed, 0 regressions (1 pre-existing, unrelated flake8 failure in `Antiquarian.py`/`FS.py`, confirmed via `git show` to predate this work). Superseded Gemini drafts deleted. Issues #13/#25 checked against the 8 renamed names - neither references any of them, no updates needed there. |
...
| 27 | 🏗️ | UI Overhaul Task 2: Voyageur Python Scripts Refactor |
...
* ✅ Root cause of "Unknown error [preauth]" after `Postponed publickey`: private key `id_ed25519_antiquarian` was **passphrase-encrypted**; BatchMode clients can't sign → client RST (10054) → server logs the crash. Server + S4U were healthy all along (proven via `-ddd` harness + 4624 audits).
...
* ✅ Removed `CENSUS_IMAGE_DIR` and `IMAGE_EXTENSION` from Global Settings (`Antiquarian.py`).
