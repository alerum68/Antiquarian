...
Converts a census CSV into a GEDCOM 5.5.1 file, tailored for either
RootsMagic (RM) or Family Tree Maker (FTM), with custom extensions for
both. Configuration comes from the environment or a .env file.
"""
...
# A raw census cell value as read from the CSV/DataFrame.
CellValue = Union[str, int, float, bool, None]
# A field in a household-unit/flag record (husband/wife/children/anchor/person/etc.).
HouseholdField = Union[pd.Series, List[pd.Series], str, float, None]

# Setting override=False creates a strict cascade:
# 1. Variables already in Memory (passed dynamically from CensusExtractor, or by the Toolbox) win.
# 2. Variables in the project root's .env (global settings) fill the blanks.
# 3. Variables in this tool's own .env (CensusConverter/.env) fill anything still missing.
# 4. If none of the above exist, the hardcoded defaults in this file are used.
...
def clean_str(val: CellValue) -> str:
...
def child_evaluation(unit: Dict[str, HouseholdField], member: pd.Series) -> Tuple[bool, float, str]:
    h = unit.get('husband')
    w = unit.get('wife')
...
def find_parent(units: List[Dict[str, HouseholdField]], member: pd.Series) -> Optional[Tuple[int, float, str]]:
...
def parse_household(group: pd.DataFrame) -> Tuple[
        List[Dict[str, HouseholdField]], List[pd.Series], List[Dict[str, HouseholdField]]]:
    members = [row for _, row in group.iterrows()]
    n = len(members)
    flags: List[Dict[str, HouseholdField]] = []
    units: List[Dict[str, HouseholdField]] = []
...
def resolve_cross_family_links(
        units: List[Dict[str, HouseholdField]], unrelated: List[pd.Series], flags: List[Dict[str, HouseholdField]]
) -> Tuple[List[Dict[str, HouseholdField]], List[pd.Series], List[Dict[str, HouseholdField]]]:
    """
...
def append_unit_if_not_empty(units: List[Dict[str, HouseholdField]], unit: Optional[Dict[str, HouseholdField]]) -> None:
...
def parse_household_relational(group: pd.DataFrame) -> Tuple[
    ...
    current_unit: Optional[Dict[str, HouseholdField]] = None
    primary_head: Optional[pd.Series] = None
    primary_spouse: Optional[pd.Series] = None
    last_child_inlaw_unit: Optional[Dict[str, HouseholdField]] = None

    head_parents_unit: Optional[Dict[str, HouseholdField]] = None
    spouse_parents_unit: Optional[Dict[str, HouseholdField]] = None
...
def build_citation_block(row: pd.Series, rec_id: str, m_id: str, real_page: str, target_software: str, row_town: str,
    ...
    if target_software == "RM":
        cit.extend([f"3 PAGE {row_roll}; {row_town}; {real_page}; {fam_num}; {person_str}", "3 _TMPLT", "4 FIELD",
                    "5 NAME RollNo", f"5 VALUE {row_roll}", "4 FIELD",
                    "5 NAME CivilDivision", f"5 VALUE {row_town}", "4 FIELD",
                    "5 NAME PageID", f"5 VALUE {real_page}", "4 FIELD",
                    "5 NAME HouseholdID", f"5 VALUE {fam_num}", "4 FIELD",
        ...
        cit.extend(
            [f"3 PAGE {person_str}; p. {real_page}, dwell. {dwell_num}, fam. {fam_num}; {row_town}; {row_county}; "
             f"{row_state}; Roll {row_roll}; Film {row_film}",
...
def get_occupation_value(row: pd.Series) -> str:
    ...
    if industry := clean_str(row.get('Industry', '')):
...
def get_master_sources(target_software: str) -> List[str]:
    if target_software == "RM":
        return [f"0 @S_{CENSUS_YEAR}_CENSUS@ SOUR", f"1 ABBR {CENSUS_YEAR} U.S. Federal Census",
                f"1 TITL , {CENSUS_YEAR} U.S. Federal Census, {COUNTY}, {STATE}, Population, , ; "
                f"on-line image {FILM_NUMBER}, .",
                f"1 _SUBQ , {CENSUS_YEAR} U.S. Federal Census, {COUNTY}, {STATE}, Population, , .",
                f"1 _BIBL {COUNTY}, {STATE}, {CENSUS_YEAR} U.S. Federal Census, Population. on-line image "
                f"{FILM_NUMBER}. {CENSUS_PUB_LOC}: {CENSUS_PUBLISHER}.",
...
def build_gedcom(df_in: pd.DataFrame, target_software: str) -> None:
    ...
    for idx, row in df.iterrows():
        ...
        # Extracted here so they're available for the citation block below.
        row_state = get_row_val(row, ['State', 'State/Province'], STATE)
        row_county = get_row_val(row, ['County', 'Parish'], COUNTY)
        row_town = get_row_val(row, ['City', 'Township', 'Town', 'Civil Division', 'Ward'], TOWNSHIP)
        row_country = get_row_val(row, ['Country'], 'USA')
