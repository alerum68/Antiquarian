...
def main():
    ...
    # from its own subfolder's .env, so CensusConverter stays runnable standalone.
    load_dotenv(Path(__file__).resolve().parent.parent / ".env", override=False)
    load_dotenv(Path(__file__).resolve().parent / ".env", override=False)

    program_dir = os.getenv("PROGRAM_DIR", "")
    url = os.getenv("CENSUS_URL", "").strip()
    csv_dir = os.getenv("CSV_DIR", "MGS_Toolbox/Working/Project/CSV")
    base_img_setting = os.getenv("CENSUS_IMAGE_DIR", "Media/Project/Census")
    gedcom_script = os.getenv("CENSUS_GEDCOM_SCRIPT", "MGS_Toolbox/CensusConverter/CensusConverter.py").strip()
