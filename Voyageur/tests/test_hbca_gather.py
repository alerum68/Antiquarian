...
def test_download_keystone_media_skips_existing_file_when_collision_is_skip(tmp_path, monkeypatch):
    """When GATHER_ON_COLLISION=skip, download_keystone_media must not re-fetch a file
    that already exists on disk."""
    monkeypatch.setenv("GATHER_ON_COLLISION", "skip")

    existing = tmp_path / "reel001.jpg"
    ...
    result = download_keystone_media(
        ["http://example.com/reel001.jpg"], tmp_path, _FakeSession()
    )

    assert fetch_calls["n"] == 0, "HTTP GET must not fire when collision=skip and file exists"
    assert existing.read_bytes() == b"cached_content", "Existing file must be unchanged"
    assert result == [str(existing)], "Existing file path must still be returned"


def test_download_keystone_media_overwrites_existing_file_when_collision_is_overwrite(tmp_path, monkeypatch):
    """When GATHER_ON_COLLISION=overwrite, download_keystone_media must re-fetch and
    replace a file that already exists on disk."""
    ...
    assert existing.read_bytes() == b"fresh_content", "File must be overwritten when collision=overwrite"
